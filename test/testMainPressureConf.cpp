// Test of main pressure configulator.
//
...
int main(int argc, char **argv) {
	...
	if (argc == 2) {
		...
		for (int i=0; i < MAX_NCYCLE; i++) {
			double input = (double)i/(double)MAX_NCYCLE;
			target.set(input);;
			double output = target.get();
			fprintf(stdout,"%lf\t%lf\n",input,output);
		}
		for (int i=0; i < MAX_NCYCLE; i++) {
			double input = (double)(MAX_NCYCLE-i)/(double)MAX_NCYCLE;
			target.set(input);;
			double output = target.get();
			fprintf(stdout,"%lf\t%lf\n",input,output);
		}
