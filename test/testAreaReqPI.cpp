// Test of area requirement PI controller.
//
...
int main(int argc, char **argv) {
	...
	if (argc == 2) {
		...
		for (int i=0; i < N_SEC*MAX_TIME; i++) {
			...
			double input = 0.0+100.0*sin(2.0*M_PI*t_s*FREQ);
			target.set(input);
		...
		for (int i=0; i < N_SEC*MAX_TIME; i++) {
			double t_s = (double)i / (double)N_SEC;
			double input = 0.0+100.0*sin(2.0*M_PI*t_s*FREQ/10.0);
			target.set(input);
			double output = target.get();
			fprintf(stdout,"%lf\t%lf\n",input,output);
		}
